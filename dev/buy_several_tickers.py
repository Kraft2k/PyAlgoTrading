    ...
    def next(self):
        ...
        for data in self.datas:  # We run through all the requested bars of all tickers
            ...
            if status in [0, 1]:
                ...
                size = random.randint(1, 30) * lot  # we will buy 1 lot - we will not check for money, we believe that they are there)
...
if __name__ == '__main__':
    ...
    clientCode = "2007693"  # Client code (assigned by the broker)
    #clientCodeForOrders = "FZQU251223A"  # Client code for orders (assigned by the broker)
    firmId = 'NC0058900000'  # Company code (assigned by the broker
    ...
    for _symbol in symbols:  # Пробегаемся по всем тикерам
        ...
        info_tickers = get_some_info_for_tickers([_symbol, ], live_prefix)  # we take information about the ticker (minimum price step, number of decimal places)
        timeframe = "M1"
        fromdate = dt.datetime.utcnow() 
        data = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=1, dataname=_symbol, fromdate=fromdate, live_bars=True, name=f"{live_prefix}{_symbol}")  # set True here - if you need to receive live bars
    ...
    # data2 = store.getdata(timeframe=bt.TimeFrame.Minutes, compression=1, dataname=symbol2, fromdate=fromdate,
    #                       live_bars=True)  # set True here - if you need to receive live bars
